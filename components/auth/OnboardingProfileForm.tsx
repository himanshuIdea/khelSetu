...
export function OnboardingProfileForm() {
  ...
  const [draft, setDraft] = useState<OnboardingDraft>(() => loadOnboardingDraft() ?? getDefaultDraft());
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col flex-1 w-full max-w-2xl">
      ...
      <SportsField
        ...
        onChange={(nextSports) => updateDraft({ sports: nextSports })}
      />
      ...
      <div className="flex flex-col sm:flex-row gap-[18px] mb-5">
        <FundingField
          ...
          onChange={(value) => updateDraft({ funding: value })}
        />
        <BrandColourField
          label={fields.brandColour.label}
          colors={fields.brandColour.colors}
          value={brandColour}
          onChange={(value) => updateDraft({ brandColour: value })}
        />
      </div>
      ...
      <div className="mt-auto flex flex-col-reverse sm:flex-row justify-between items-stretch sm:items-center gap-3 pt-8">
        ...
        <div className="flex flex-col items-stretch sm:items-end gap-1.5">
          ...
          <AuthContinueButton
            label={isSubmitting ? "Creating academy…" : onboarding.continueLabel}
            type="submit"
