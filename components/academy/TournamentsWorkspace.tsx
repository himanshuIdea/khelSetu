    ...
    <div className="flex flex-col flex-1 min-h-0 min-w-0 w-full lg:overflow-hidden">
      ...
      {tournament && tournamentHeaderMatches ? (
        <div className="shrink-0 bg-card border border-line rounded-(--radius) shadow-card p-5 mb-4 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-[18px]">
          ...
          <div className="flex flex-col items-stretch sm:items-end gap-2 self-start sm:self-auto shrink-0 min-w-0">
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              {tournament.status === "live" ? (
                <button
                  ...
                  className="min-h-[36px] px-3 py-1.5 rounded-[8px] border border-line text-[12px] font-semibold text-ink hover:bg-surface"
                >
                  End tournament
                </button>
              ) : null}
      ...
      {!tournament ? (
        ...
        <SplitLayout className="flex-1 min-h-0 lg:overflow-hidden">
          ...
          <SidePanel className="lg:min-h-0 lg:max-h-full">
            <div className="bg-card border border-line rounded-(--radius) shadow-card p-[18px] lg:max-h-full lg:overflow-y-auto overscroll-y-contain [-webkit-overflow-scrolling:touch] space-y-4">
              {liveMatch ? (
                <div className="p-3 border border-red/30 bg-red/5 rounded-[11px]">
                  ...
                  {liveMatch.matLabel ? (
                    <div className="text-[11px] text-muted mt-1">{liveMatch.matLabel}</div>
                  ) : null}
                </div>
              ) : null}

              <div>
                <SectionTitle title="Next up" subtitle="First scheduled bout on timeline" />
                {nextUp ? (
                  <div className="mt-3 p-3 border border-line rounded-[11px]">
                  ...
                  <p className="text-[11.5px] text-muted mt-3">No bouts scheduled yet.</p>
                )}
              </div>

              <div>
                <SectionTitle title="Today's timeline" subtitle="Quick preview" />
              ...
              <div className="pt-3 border-t border-line2 flex justify-between">
                <div>
