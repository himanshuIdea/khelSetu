      ...
      <SplitLayout>
        <div className="flex-1 min-w-0 animate-pulse" aria-busy aria-label="Loading teams">
          ...
          <div className="border border-line rounded-(--radius) overflow-hidden">
            ...
            {Array.from({ length: 4 }).map((_, i) => (
              ...
              >
                <div className="w-9 h-9 rounded-full bg-line2 shrink-0" />
                <div className="h-3.5 w-36 bg-line2 rounded" />
                <div className="h-3 w-16 bg-line2 rounded hidden sm:block" />
                <div className="h-6 w-20 bg-line2 rounded-full hidden md:block" />
              </div>
            ))}
          </div>
        </div>
        ...
        <SidePanel className="animate-pulse">
          <div className="bg-card border border-line rounded-(--radius) p-4 space-y-3">
            <div className="h-4 w-24 bg-line2 rounded" />
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="flex gap-3 items-center py-2">
                <div className="w-9 h-9 rounded-[10px] bg-line2 shrink-0" />
